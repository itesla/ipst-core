...
public class Nio2VirtualCaseScanner implements Nio2ProjectFileScanner {

    @Override
    public Class<? extends ProjectFile> getType() {
        return Nio2VirtualCase.class;
    }

    @Override
    public Collection<ProjectNode> scan(Nio2ProjectFolder parent, Path path) {
        if (Files.isDirectory(path)) {
            Path metadataFile = path.resolve(Nio2VirtualCase.Metadata.XML_FILE_NAME);
            if (Files.exists(metadataFile)) {
                return Collections.singleton(new Nio2VirtualCase(path, parent));
            }
        }
